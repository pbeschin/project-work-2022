...
char* topicPrezzo = "pw2k22/+/pagamento";
char topic[40];
...
void setup() {
  ...
  attachInterrupt(digitalPinToInterrupt(INTERRUPT_PIN), pagato, RISING);
...
void loop() {
  ...
  if(pagamento) {
    if(daPagare){
      daPagare=false;
      client.publish("pw2k22/1.1/pagato", "1");
      lcd.clear();
      lcd.print("PAGATO");
    }
    pagamento = false;  
